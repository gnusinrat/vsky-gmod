...
-- Уникальные ConVar для сервера VSKY, чтобы избежать конфликтов с оригиналом
local sv_npctranqgun_hitdamage = CreateConVar( "sv_npctranqgun_hitdamage_vsky", "0", ( FCVAR_REPLICATED + FCVAR_ARCHIVE ), "Сколько урона наносят дротики транквилизатора?", 0, 100 )
local sv_npctranqgun_sleeptime = CreateConVar( "sv_npctranqgun_sleeptime_vsky", "30", ( FCVAR_REPLICATED + FCVAR_ARCHIVE ), "Время сна усыпленной цели в секундах.", 0, 600 )
local sv_npctranqgun_knockouttime = CreateConVar( "sv_npctranqgun_knockouttime_vsky", "3", ( FCVAR_REPLICATED + FCVAR_ARCHIVE ), "Время перед тем, как цель потеряет сознание после попадания.", 0, 60 )
local sv_npctranqgun_physdmgthreshold = CreateConVar( "sv_npctranqgun_physdmgthreshold_vsky", "10", ( FCVAR_REPLICATED + FCVAR_ARCHIVE ), "Порог физического урона для рэгдолла.", 0, 1000 )
local sv_npctranqgun_dropweapon = CreateConVar( "sv_npctranqgun_dropweapon_vsky", "1", ( FCVAR_REPLICATED + FCVAR_ARCHIVE ), "Должна ли цель бросать оружие при отключении?", 0, 1 )
local sv_npctranqgun_onlyfleshnpcs = CreateConVar( "sv_npctranqgun_onlyfleshnpcs_vsky", "1", ( FCVAR_REPLICATED + FCVAR_ARCHIVE ), "Транквилизатор действует только на органику?", 0, 1 )

local cl_npctranqgun_playsnoringsnds = CreateClientConVar( "cl_npctranqgun_playsnoringsnds_vsky", "1", true, true, "Воспроизводить ли звуки храпа усыпленных целей?", 0, 1 )
local cl_npctranqgun_draweffect = CreateClientConVar( "cl_npctranqgun_draweffect_vsky", "1", true, false, "Рисовать ли эффект 'Zzz' над головой спящих?", 0, 1 )
...
-- Хук принудительного воспроизведения анимаций падения и подъема для игроков
hook.Add( "CalcMainActivity", "VSKY_Tranq_PlayerAnimOverride", function( ply, vel )
    if ply:GetNWBool("VSKY_Tranq_IsFalling") then
        local anim = ply:GetNWString("VSKY_Tranq_FallAnim")
        if anim ~= "" then
    ...
    elseif ply:GetNWBool("VSKY_Tranq_IsRising") then
        local anim = ply:GetNWString("VSKY_Tranq_RiseAnim")
        if anim ~= "" then
            local seq = ply:LookupSequence( anim )
            if seq and seq > 0 then
                return -1, seq
...
-- Принудительное продвижение кадров анимации, даже когда игрок заморожен (Freeze)
hook.Add( "UpdateAnimation", "VSKY_Tranq_PlayerAnimUpdate", function( ply, vel, maxseqgroundspeed )
    if ply:GetNWBool("VSKY_Tranq_IsFalling") or ply:GetNWBool("VSKY_Tranq_IsRising") then
        ply:FrameAdvance( FrameTime() )
        return true -- Предотвращаем стандартное затухание/замедление анимации от скорости движения
    end
end )
...
if ( SERVER ) then
        ...
        local function OnLambdaInitialize( lambda, weapon )
            ...
            function lambda:Tranquilized()
                ...
                for _, ply in ipairs( GetHumans() ) do
                    if ply:GetInfoNum( "cl_npctranqgun_playsnoringsnds_vsky", 0 ) != 1 then continue end
                ...
                if IsValid( ragdoll ) then
                    ...
                    if TraceLine( traceTbl ).Hit then
                        ...
                        for i = 0, ( ragdoll:GetPhysicsObjectCount() - 1 ) do
                            ...
                            ragMoveData[ #ragMoveData + 1 ] = { phys, physBone, phys:GetPos(), phys:GetAngles() }
                        end
    ...
    -- Хук очистки сна при смерти игрока, предотвращающий баги с невидимостью и зависанием таймеров
    hook.Add( "PlayerDeath", "VSKY_Tranq_PlayerDeathCleanup", function( ply )
        ply:SetNWBool("VSKY_Tranq_IsFalling", false)
        ply:SetNWBool("VSKY_Tranq_IsRising", false)
        ply:SetNWString("VSKY_Tranq_FallAnim", "")
        ply:SetNWString("VSKY_Tranq_RiseAnim", "")
        ply.l_TranqGun_IsTranquilized = false
        ply:Freeze( false )
        ply:SetNoDraw( false )
        ply:SetNotSolid( false )
        ply:SetViewEntity( nil )

        local createID = ply:AccountID() or ply:EntIndex()
        timer.Remove( "VSKY_PlayerFallAnimTimer_" .. createID )
        timer.Remove( "VSKY_PlayerRiseAnimTimer_" .. createID )
        timer.Remove( "VSKY_PlayerWakeUpTimer_" .. createID )
        timer.Remove( "VSKY_PlayerKnockOutTimer_" .. createID )

        local rag = ply.l_TranqGun_Ragdoll
        if IsValid( rag ) then
            if rag.l_TranqGun_SnoreSnd then
                rag.l_TranqGun_SnoreSnd:Stop()
            end
            rag:Remove()
        end
    end )
    ...
    local function OnServerThink()
        ...
        for _, ent in ipairs( ents_GetAll() ) do
            -- Обычный цикл сна на сервере обрабатывает только NPC и Лямбд. Игроки просыпаются через выделенные таймеры.
            if !IsValid( ent ) or !ent.l_TranqGun_IsTranquilized or ent:IsPlayer() then continue end

            local ragdoll = ent.l_TranqGun_Ragdoll
            if !IsValid( ragdoll ) then
                if ent:IsNPC() then ent:Remove() end
                continue 
            end
            ...
            local wakeUp = false
            if !wakingTime then
                if ent.l_TranqGun_LastCollisionGroup then ent:SetCollisionGroup( ent.l_TranqGun_LastCollisionGroup ) end
                ...
                if groundTr.Hit then
                    ...
                    for i = 0, ( ragdoll:GetPhysicsObjectCount() - 1 ) do
                        ...
                        ent.l_TranqGun_PreWakePhysData[ #ent.l_TranqGun_PreWakePhysData + 1 ] = { phys, physBone, phys:GetPos(), phys:GetAngles() }
                    end
                end
            ...
            if wakeUp then
                ...
                if ent.l_TranqGun_HiddenChildren then
                    for _, child in ipairs( ent.l_TranqGun_HiddenChildren ) do
                        if !IsValid( child ) then continue end
                        child:SetRenderMode( RENDERMODE_NORMAL )
                        child:DrawShadow( true )
                    end
    ...
    local function DrawZZZs()
        ...
        for _, ragdoll in ipairs( ents_GetAll() ) do
            ...
            if !iconData then
                -- Цвет эффекта сна Zzz теперь полностью совпадает с цветом игрока
    ...
    local function AddToolMenuTabs()
        spawnmenu.AddToolCategory( "Utilities", "ratbar1999", "ratbar1999" )
    end
    ...
    local function PopulateToolMenu()
        spawnmenu.AddToolMenuOption( "Utilities", "ratbar1999", "TranqGunMenu", "Tranquilizer Gun (VSKY)", "", "", function( panel ) 
            local preset = panel:ToolPresets( "npctranqgun", cvarList )
            panel:NumSlider( "Dart Hit Damage", "sv_npctranqgun_hitdamage_vsky", 0, 100, 0 )
            panel:ControlHelp( "Сколько урона наносят дротики транквилизатора? Масштабируется в зависимости от зоны попадания." )
            panel:NumSlider( "Sleep Time", "sv_npctranqgun_sleeptime_vsky", 0, 600, 0 )
            panel:ControlHelp( "Как долго длится сон до того, как жертва проснется." )
            panel:NumSlider( "Pass Out Time", "sv_npctranqgun_knockouttime_vsky", 0, 60, 1 )
            panel:ControlHelp( "Как долго жертва остается на ногах до того, как упадет." )
            panel:NumSlider( "Physical Damage Threshold", "sv_npctranqgun_physdmgthreshold_vsky", 0, 1000, 0 )
            panel:ControlHelp( "Порог урона физики, достаточный для нанесения урона спящему." )
            panel:CheckBox( "Drop Weapon", "sv_npctranqgun_dropweapon_vsky" )
            panel:ControlHelp( "Должно ли выпадать оружие у целей при отключении?" )
            panel:CheckBox( "Fleshy NPCs Only", "sv_npctranqgun_onlyfleshnpcs_vsky" )
            panel:ControlHelp( "Действовать только на органических существ?" )
            panel:CheckBox( "Play Snoring Sounds", "cl_npctranqgun_playsnoringsnds_vsky" )
            panel:ControlHelp( "Проигрывать звук храпа спящих целей?" )
            panel:CheckBox( "Draw Sleep Effect", "cl_npctranqgun_draweffect_vsky" )
            panel:ControlHelp( "Рисовать над головой спящей цели значок сна Zzz?" )
        end )
    end
        ...
        if hitFlesh or !sv_npctranqgun_onlyfleshnpcs:GetBool() then
            ...
            elseif ent:Health() > 0 then
                ...
                elseif ent:IsPlayer() and ent:Alive() then
                    local createID = ent:AccountID() or ent:EntIndex()
                    local koTimer = "VSKY_PlayerKnockOutTimer_" .. createID

                    if !TimerExists( koTimer ) then
                        ent:ScreenFade( SCREENFADE.IN, Color( 0, 0, 0, 150 ), koTime, 0 )

                        CreateTimer( koTimer, koTime, 1, function()
                            if !IsValid( ent ) or !ent:Alive() then return end

                            -- ФАЗА 1: Кинематографичная анимация падения игрока (Slow collapse)
                            ent:SetNWBool("VSKY_Tranq_IsFalling", true)
                            ent:SetNWString("VSKY_Tranq_FallAnim", "death_0" .. random( 4 ))
                            ent:Freeze( true ) -- Блокируем движение
                            
                            local rndPitch = random( 95, 110 )
                            ent:EmitSound( "lambdaplayers/weapons/tranqgun/vo_presleep" .. random( 7 ) .. ".mp3", 70, rndPitch, 1, CHAN_VOICE )

                            -- Таймер на длительность анимации падения перед рэгдоллом
                            local fallDuration = 1.6
                            CreateTimer( "VSKY_PlayerFallAnimTimer_" .. createID, fallDuration, 1, function()
                                if !IsValid( ent ) or !ent:Alive() then return end
                                
                                ent:SetNWBool("VSKY_Tranq_IsFalling", false)
                                ent:SetNWString("VSKY_Tranq_FallAnim", "")
                                
                                -- ФАЗА 2: Вход в состояние сна (создание рэгдолла и отключение физического тела)
                                ent:ScreenFade( SCREENFADE.OUT, Color( 0, 0, 0, 255 ), 1, sv_npctranqgun_sleeptime:GetFloat() )
                                ent:SetNoDraw( true )
                                ...
                                -- Таймер удержания во сне
                                CreateTimer( "VSKY_PlayerWakeUpTimer_" .. createID, sv_npctranqgun_sleeptime:GetFloat(), 1, function()
                                    if !IsValid( ent ) then return end

                                    if IsValid( ent.l_TranqGun_Ragdoll ) then
                                        ent:SetPos( ent.l_TranqGun_Ragdoll:GetPos() )
                                        if ent.l_TranqGun_Ragdoll.l_TranqGun_SnoreSnd then
                                            ent.l_TranqGun_Ragdoll.l_TranqGun_SnoreSnd:Stop()
                                        end
                                        ent.l_TranqGun_Ragdoll:Remove()
                                    end
                                    ent:SetViewEntity( nil )

                                    -- ФАЗА 3: Пробуждение игрока с красивой анимацией подъема (Rise animation)
                                    ent:SetNoDraw( false )
                                    ent:DrawViewModel( false ) -- ViewModel пока скрыт, пока игрок встает
                                    
                                    ent:SetNWBool("VSKY_Tranq_IsRising", true)
                                    ent:SetNWString("VSKY_Tranq_RiseAnim", "zombie_slump_rise_0" .. (random(2) == 1 and "1" or "2_fast"))

                                    -- Таймер на завершение анимации вставания с земли
                                    local riseDuration = 1.8
                                    CreateTimer( "VSKY_PlayerRiseAnimTimer_" .. createID, riseDuration, 1, function()
                                        if !IsValid( ent ) then return end

                                        ent:SetNWBool("VSKY_Tranq_IsRising", false)
                                        ent:SetNWString("VSKY_Tranq_RiseAnim", "")
                                        ent:SetNotSolid( false )
                                        ent:Freeze( false ) -- Возвращаем управление
                                        ent:DrawViewModel( true )
                                        local activeWep = ent:GetActiveWeapon()
                                        if IsValid(activeWep) then activeWep:SetNoDraw(false) end

                                        ent.l_TranqGun_IsTranquilized = false
                                        ent:ScreenFade( SCREENFADE.IN, Color( 0, 0, 0, 255 ), 2, 0 ) -- Прояснение экрана
                                    end )
                                end )
                            end )
                        end )
                ...
                elseif !ent:IsNextBot() and ent:IsNPC() and ( IsValidRagdoll( ent:GetModel() ) or IsValidProp( ent:GetModel() ) ) then
                    ...
                    if !TimerExists( koTimer ) then
                        local function BecomeADoll( ent, ragCopyTarg )
                            ...
                            if soundTbl then
                                for _, snd in ipairs( soundTbl ) do ent:StopSound( snd ) end
                        ...
                        if NPCVC and random( 100 ) <= ent.NPCVC_ChanceToSpeak then
...
-- Уникальное имя функции для исключения конфликтов
function M9TranqGun_VSKY_FireDart( pos, fwd, owner, weapon )
